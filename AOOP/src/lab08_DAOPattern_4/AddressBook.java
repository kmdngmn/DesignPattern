...
public class AddressBook {
	public static void main(String[] args) {
		...
		PersonDao personDaoSQL = new PersonDaoSQLImpl(); // SQLite로 변경하면 이 부분만 변경되고 나머지 코드는 변경되지 않음
...
//		System.out.println("\n\n===========================\n\n");
		...
		System.out.println("--- inserting...");
		p = new Person("Seonyoung Kim", "1 Hongji Dong");
		personDaoSQL.insert(p);
		p = new Person("Jangkwon Lee", "2 Hongji Dong");
		personDaoSQL.insert(p);

		System.out.println("--- finding all...");
		for (Person pi : personDaoSQL.findAll()) {
			System.out.println("reading... " + pi);
		}

		System.out.println("--- updating...");
		p = personDaoSQL.findAll().get(0);
		p.setName("Sooyoung Lim");
		personDaoSQL.update(p, p.getId());

		System.out.println("--- see if updated...");
		p = personDaoSQL.findById(1);
		if (p != null) {
			System.out.println(p);
		}

		System.out.println("--- deleting...");
		personDaoSQL.delete(1); // or personDaoSQL.delete(p);
