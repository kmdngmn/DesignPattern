...
//		labelWindow = new LabelWindow(LABEL_WINDOW_TITLE, X, Y + (HEIGHT + GAP) * 2, WIDTH, HEIGHT);
		...
		textFieldWindow = TextFieldWindowFactory.getInstance().createWindow(TEXTFIELD_WINDOW_TITLE, X, Y + HEIGHT + GAP,
				WIDTH, HEIGHT);
		labelWindow = LabelWindowFactory.getInstance().createWindow(LABEL_WINDOW_TITLE, X, Y + (HEIGHT + GAP) * 2,
				WIDTH, HEIGHT);
