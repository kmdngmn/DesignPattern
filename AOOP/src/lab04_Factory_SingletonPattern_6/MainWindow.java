...
//		labelWindow = new LabelWindow(LABEL_WINDOW_TITLE, X, Y + (HEIGHT + GAP) * 2, WIDTH, HEIGHT);
		...
		textFieldWindow = TextFieldWindowFactory.getInstance().creatWindow(TEXTFIELD_WINDOW_TITLE, X,
				Y + HEIGHT + GAP, WIDTH, HEIGHT);
		labelWindow = LabelWindowFactory.getInstance().creatWindow(LABEL_WINDOW_TITLE, X, Y + (HEIGHT + GAP) * 2,
				WIDTH, HEIGHT);
