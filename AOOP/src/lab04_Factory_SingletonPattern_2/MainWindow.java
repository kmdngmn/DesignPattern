...
public class MainWindow extends FrameWindow implements ActionListener {
	...
	public MainWindow(String title) {
		super(null, title, X, Y, WIDTH, HEIGHT);
...
//		labelWindow = new LabelWindow(LABEL_WINDOW_TITLE, X, Y + (HEIGHT + GAP) * 2, WIDTH, HEIGHT);
		...
		textFieldWindow = FactoryConcrete.getInstance().createWindow("TextField", TEXTFIELD_WINDOW_TITLE, X, Y + HEIGHT + GAP, WIDTH,
				HEIGHT);
		labelWindow = FactoryConcrete.getInstance().createWindow("Label", LABEL_WINDOW_TITLE, X, Y + (HEIGHT + GAP) * 2, WIDTH,
				HEIGHT);
