...
public class MainWindow extends FrameWindow {
	...
	public JFrame createWindow(String title, int x, int y, int width, int height) {
		...
		// createPanel() 함수를 호출해서 파일에서 읽은 디스플레이 구성 요소에 따라 디스플레이를 조합하고, 전체 디스플레이를 구성하는
		// 패널을 돌려 받음
	...
	public JPanel createPanel(int width, int height) {
		...
		// 리스트에 있는 내용을 바탕으로 장식 추가하기. 장식을 추가하는 것은 현재까지 구성된 디스플레이 패널을 감쌀 수 있는 새로운 패널을
		// 생성하고,
		...
		for (String name : displayList) {
			if (name.equals("speed")) {
				...
				// display 변수가 참조하는 기존 디플레이에 자동차 번호판 디스플레이를 덧 붙임
				display = new CarNumberDisplay(display, WIDTH, HEIGHT);
			}
		}
	...
	public static void main(String[] args) throws IOException {
		// displays.txt 파일에서 디스플레이 구성 요소의 내용을 읽어서 list에 저장할 예정
		ArrayList<String> list;
		// LoadHudDisplays 클래스에서는 파일 이름을 입력으로 전달 받고 이름을 저장해둠
		LoadHudDisplays loadDisplay = new LoadHudDisplays("displays.txt");
		// load() 함수를 호출해서 앞에서 지정했던 파일을 읽고, 리스트 형태로 구성해서 반환
		list = loadDisplay.load();

		// 디버깅 목적으로 작성된 코드. 파일의 내용이 제대로 읽혔는지 확인
		for (String s : list) {
			System.out.println(s);
		}

		// 전체 디스플레이 화면을 구성하고 보여줌. 디스플레이 구성 요소를 나타내는 list를 입력으로 전달함
