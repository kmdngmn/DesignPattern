...
//		labelWindow = new LabelWindow(LABEL_WINDOW_TITLE, X, Y + (HEIGHT + GAP) * 2, WIDTH, HEIGHT);
		...
		textFieldWindow = CreateWindow.getInstance().windowCreator("JTextFiled", TEXTFIELD_WINDOW_TITLE, X,
				Y + HEIGHT + GAP, WIDTH, HEIGHT);
		labelWindow = CreateWindow.getInstance().windowCreator("JLabel", LABEL_WINDOW_TITLE, X, Y + (HEIGHT + GAP) * 2,
				WIDTH, HEIGHT);
